...
# ─── НОВОЕ: переменные для роутинга заказов ───
KWORK_URL = os.getenv("KWORK_URL", "https://kwork.ru/user/artem_sh")

conversation_history = {}

# ═══ СИСТЕМНЫЙ ПРОМПТ ЛИЛЫ ═══ (оригинальный — не тронут)
...
# ══════════════════════════════════════════════
# ─── НОВОЕ: ЛИЛА ФИЛЬТРУЕТ ЗАКАЗЫ ОТ БОТОВ ───
# ══════════════════════════════════════════════

async def lilu_review_job(job: dict, source_bot: str) -> dict:
    """
    Лила получает заказ от Полифана или Карточника.
    Переводит на русский, объясняет, решает: пропустить или отклонить.
    """
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            r = await client.post(
                ...
                json={
                    "model": "llama-3.3-70b-versatile",
                    "messages": [
                        {"role": "system", "content": LILU_SYSTEM},
...
async def lilu_send_approved_job(bot, job: dict, review: dict, source_bot: str):
    """Лила красиво оформляет одобренный заказ и шлёт Артёму"""
...
async def process_incoming_job(bot, job: dict, source_bot: str):
    ...
    Лила анализирует и либо молча отклоняет, либо красиво шлёт Артёму.
    """
...
async def skills_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        " • Инфографика с фото товара\n\n"
        "💰 *Анастасия* — бухгалтер:\n"
        " • Учёт доходов по всем биржам\n"
        " • Ежедневные отчёты\n\n"
        "👑 *Лила* — CEO:\n"
        " • Фильтрация заказов (перевод + анализ)\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── НОВЫЕ КНОПКИ: одобрение/отклонение заказов от ботов ───
...
# ═══ ОБРАБОТЧИК СООБЩЕНИЙ ═══ (оригинальный — не тронут)
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if update.message.photo:
            ...
            # ─── НОВОЕ: приём заказов от Полифана и Карточника ───
            if update.message.text.startswith("🤖JOB:"):
                try:
                    ...
                    await update.message.reply_text(
                        f"📨 Получила от {source_bot}. Анализирую..."
                    )
        ...
        # Режим проверки текста (оригинальный)
        ...
        # Режим правки от Лилы (оригинальный)
        ...
        # Обычный разговор (оригинальный)
        reply = await get_lilu_response(user_id, user_text, image_b64)

        # Голосовой ответ (оригинальный)
        try:
...
# ═══ ПОЛУЧЕНИЕ РАБОТ НА ПРОВЕРКУ ═══ (оригинальная — не тронута)
...
# ═══ ЗАПУСК ═══ (оригинальный + новые команды)
def main():
