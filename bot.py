...
def auto_extract_memory(user_msg: str, lila_msg: str):
    ...
    if not any(kw in combined.lower() for kw in keywords):
        return
    try:
        # Простое извлечение — первые 200 символов сообщения пользователя
...
async def maybe_search(query: str) -> str | None:
    """Возвращает результат поиска если запрос требует актуальных данных."""
...
async def analyze_photo(photo_url: str, user_question: str = "") -> str:
    """Анализирует фото через GPT Image API."""
    if not KIE_API_KEY:
...
async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner or premium:
        info = f"{'👑 Владелец' if is_owner else f'💎 Премиум до {data[chr(112)+chr(114)+chr(101)+chr(109)+chr(105)+chr(117)+chr(109)+chr(95)+chr(117)+chr(110)+chr(116)+chr(105)+chr(108)]}' }\nЗапросов: безлимит"
    else:
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверка лимита для не-владельцев
    if not is_owner:
        data = await check_and_reset_daily(user.id)
        premium = await is_premium_active(data)
    ...
    # Поиск в памяти
    memories = search_memory(user_msg, n=4)

    # Авто-поиск в интернете если нужно
    ...
    asyncio.create_task(asyncio.to_thread(auto_extract_memory, user_msg, response))
...
async def handle_voice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Отвечаем
    memories = search_memory(user_text, n=3)
...
async def proactive_message(app):
    """Раз в 1-3 дня Лила пишет сама."""
    await asyncio.sleep(60)  # Первый раз через минуту после запуска
    while True:
        try:
            if YOUR_CHAT_ID:
        ...
        # Следующее через 1-3 дня (случайно)
